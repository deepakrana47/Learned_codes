...
class SimpleRNN:
    ...
    def generate(self, pi, word2idx):
        idx2word = {v:k for k,v in word2idx}
        V = len(pi)
...
def train_poetry():
    ...
    rnn.fit(sentences, learning_rate=10e-5, show_fig=True, epochs=400, activation=T.nnet.relu)
    rnn.save("RNN_D30_M30_epochs400_relu.npz")

def generate_poetry():
    sentences, word2idx = get_robert_frost()
    rnn = SimpleRNN.load("RNN_D30_M30_epochs2000_relu.npz", activation=T.nnet.relu)
